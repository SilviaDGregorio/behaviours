...
namespace Interfaz{
	...
	static PyObject *ab_manager_resolve(PyObject *self, const std::string &name);
...
*/
	...
	PyObject *PyInit_ab(void){
		auto m = Py_InitModule("ab", ab_methods);
		if (!m)
			return NULL;
		manager_error = PyErr_NewException((char*)"ab.exception", nullptr, nullptr);
